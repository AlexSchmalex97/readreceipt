...
FOR SELECT 
USING (true)
-- This policy will be restricted by the columns we select in our queries
-- We'll update application code to only select safe fields;

-- Keep the existing policy for users to view their own complete profile
-- (Users can view their own complete profile policy already exists)
