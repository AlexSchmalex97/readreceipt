    ...
    (async () => {
      try {
        ...
        const { data: profileData, error: profileError } = await supabase
          .from("profiles")
          .select("id, username, display_name, avatar_url, color_palette, top_five_books, background_type, active_background_id, background_image_url, background_tint")
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {book.started_at && (
                            <p className="text-xs text-muted-foreground mt-2">
                              Started {(() => {
                                const d = /^\d{4}-\d{2}-\d{2}$/.test(book.started_at)
                                  ? new Date(book.started_at + "T00:00:00")
